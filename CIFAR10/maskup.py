...
class Mask:
    ...
    def cal_prune(self,index,epoch):
        if index<8:
            ind=convlayer.index(index)
            code=self.codebook[convlayer[ind+1]].view(self.model_size[convlayer[ind+1]][0],-1)
            ...
            prunni=pruni.size()[0]
            self.pruncc[index]=self.pruncc[index]+math.exp(-(epoch-37)/5)*(self.model_size[index][0]-prunni)/(self.model_size[convlayer[ind+1]][0]-prunn)
            ...
            if epoch>60:
                aphla=0.0005
            sumbook=torch.sum(self.codebook[8],dim=1)
            prun=torch.where(sumbook<20)[0]
            prunn=prun.size()[0]
    ...
    def do_growth_ww(self,epoch):
        for index in convlayer:
            if index<8:
                ...
                gr_index=np.where(self.his_gro_count[index]>18)[0]
